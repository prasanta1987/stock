...
const mstat = document.querySelector('.mstat')
...
const getMarketStatus = async () => {
    try {
        ...
        mstat.innerHTML = data.marketState[0].marketStatus
...
const buildCards = async (symbol) => {
    ...
    try {
        ...
        if (Object.keys(data).length > 0) {
            ...
            let retAginstInv = (sellPrice - (avgBuyPrice * sellQty)).toFixed(2); // Total Return
            retAginstInv = (isNaN(retAginstInv)) ? 0 : retAginstInv
            let invested = (isNaN(avlShare * avgBuyPrice)) ? 0 : (avlShare * avgBuyPrice).toFixed(2)
            ...
            if (document.querySelector(`.${symbol}`)) {
                ...
                document.querySelector(`.${symbol}-pchange`).innerHTML = `${pChnage} %`
                document.querySelector(`.${symbol}-vwap`).innerHTML = vwap
